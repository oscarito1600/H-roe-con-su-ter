...
enemigosDerrotados++;
monedas++; // ✅ Cada enemigo = 1 moneda
localStorage.setItem("monedas", monedas); // Guardar en almacenamiento local
monedasTexto.textContent = monedas; // Mostrar en pantalla
enemigosTexto.textContent = enemigosDerrotados;
// 🚀 Aumentar velocidad cada 6 enemigos
if (enemigosDerrotados % 10 === 0) {
  velocidadEnemigoBase += 0.4;
...
const enemigosConfig = [
  ...
  { imagen: "Armadura.gif", sonido: "enemySound6", vida: 2, altura: 140, ancho:160 },
  ...
  { imagen: "enemy9.gif", sonido: "enemySound9", vida: 3, altura: 105, ancho:90 },
    ...
    { imagen: "calabaza.gif",sonido: "enemySound11", vida: 3,  altura: 110, ancho: 90,  disparador: true  },
     { imagen: "noob de lava.gif", sonido: "enemySound12", vida: 3, altura: 90, ancho:90 },
     {imagen: "auto2.png", sonido: "enemySound13", vida: 3,altura: 98, ancho: 90,},
      { imagen: "Pollo goma.gif", sonido: "enemySound14", vida: 3, altura: 90, ancho:90 },
      {imagen: "enemy6.gif", sonido: "enemySound15", vida: 2,altura: 100, ancho: 100,  caeDelCielo: true},
       { imagen: "Tumba.gif",sonido: "enemySound16", vida: 2, altura: 100,ancho: 100, saleDelSuelo: true},
  { imagen: "Tanque.png",sonido: "enemySound11", vida: 3,  altura: 150, ancho: 120,  disparador: true  }
];
...
if (config.saleDelSuelo) {
  let x = Math.random() * (game.clientWidth - config.ancho);
  let sueloY = 40; // nivel del suelo
  ...
  // Colocar enemigo escondido bajo el suelo
  enemy.style.left = x + "px";
  enemy.style.bottom = "-" + config.altura + "px"; // inicia oculto
  game.appendChild(enemy);

  // Espera 1 segundo de advertencia
  setTimeout(() => {
    if (warning.parentNode) warning.remove();
    ...
    const subir = setInterval(() => {
      ...
      if (y >= sueloY) {
        ...
        // ✅ Una vez fuera, empieza a caminar
        let direccion = Math.random() < 0.5 ? -1 : 1;
        const speed = direccion * velocidadEnemigoBase;

        // ✅ Ajustar animación UNA sola vez según la dirección
        enemy.style.transform = direccion === -1 ? "scaleX(-1)" : "scaleX(1)";
        ...
        const mover = setInterval(() => {
          ...
          if (posX < -50 || posX > game.clientWidth + 50) {
            enemy.remove();
            clearInterval(mover);
          }
        }, 30);
  ...
  return; // ⚠️ detenemos aquí para que no ejecute la lógica normal
}
...
window.onload = () => {
  ...
  iniciarSpawnEnemigos();
    monedasTexto.textContent = monedas; // ✅ Mostrar monedas guardadas
